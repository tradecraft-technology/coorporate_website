...
- `assets/scene.js` adds a restrained, masked foliage breeze with WebGL and gentle pointer parallax. It renders at the display's pixel density, with a 30 fps cap and an approximately 9-megapixel drawing budget for larger displays. The static image remains the fallback when WebGL is unavailable.
...
Retina verification covers 1440×900 at 2× density and 390×844 at 3× density. The canvas now renders at 2928×1848 and 1230×2592 respectively, including the scene's overscan. Previously it rendered at 2196×1386 and 410×864. Larger viewport checks exercise the drawing budget. The 1672×941 landscape source is unchanged and still limits the finest visible detail; increasing canvas resolution does not add detail to the artwork.
